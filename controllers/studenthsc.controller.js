...
controller.getStudenthscByAdmission = async (req, res) => {
    try {
        ...
        console.log("Searching Admission:", admissionNumber);
        console.log("Searching Academic Year:", academicYear);

        const student = await Studenthsc.findOne({
            where: {
                admissionNumber: admissionNumber.trim(),
                academicYear: academicYear.trim(),   // ✅ EXACT MATCH
        ...
        if (!student) {
            return res.status(404).json({
                error: "Student not found for selected Academic Year"
            });
        }
        ...
        console.error("Error fetching HSC student:", error);

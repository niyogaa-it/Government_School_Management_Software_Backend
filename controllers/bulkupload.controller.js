...
// ─── Helper: normalize row keys — strip * and trim spaces from header names ───
const normalizeRow = (row) => {
    const normalized = {};
    for (const key of Object.keys(row)) {
        const cleanKey = key
            .replace(/★/g, "")           // remove ★
            .replace(/\(.*?\)/g, "")      // remove (Required), (YYYY-MM-DD), (JSON array), etc.
            .replace(/▼/g, "")            // remove dropdown marker
            .replace(/\*/g, "")           // remove * (legacy)
            .trim();
...
controller.bulkUploadHSC = async (req, res) => {
    try {
        ...
        // Pick data sheet — prefer "HSC_Students", skip helper sheets starting with _
        const sheetName = workbook.SheetNames.find(n => n === "HSC_Students") || workbook.SheetNames.find(n => !n.startsWith("_")) || workbook.SheetNames[0];
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                // ── Normalize group_subjects: "12,13" → ["12","13"] ──────────────────────
                let group_subjects = row.group_subjects || null;
                if (group_subjects !== null) {
                    if (typeof group_subjects === "string") {
                        // "12,13" → ["12","13"]  (what the DB JSON column expects)
                        group_subjects = group_subjects.split(",").map(s => s.trim()).filter(Boolean);
                    } else if (!Array.isArray(group_subjects)) {
                        group_subjects = [String(group_subjects)];
                    }
                    group_subjects = group_subjects.map(String);
                }

                // ── Normalize academicHistory: JSON string → array ───────────────────────
                let academicHistory = row.academicHistory || null;
                if (typeof academicHistory === "string") {
                    try {
                        academicHistory = JSON.parse(academicHistory);
                        if (typeof academicHistory === "string") academicHistory = JSON.parse(academicHistory);
                    } catch (_) { academicHistory = null; }
                }
                if (!Array.isArray(academicHistory)) academicHistory = academicHistory ? [academicHistory] : [];

                if (!Array.isArray(academicHistory)) academicHistory = academicHistory ? [academicHistory] : [];

                // ── Normalize age: "17y 8m 6d" → {years:17, months:8, days:6} ──────────
                let age = null;
                if (row.age) {
                ...
                await Studenthsc.create({
                    ...
                    academicHistory,                                        // ← FIXED: was missing before
...
controller.bulkUploadSSLC = async (req, res) => {
    try {
        ...
        const workbook = XLSX.read(req.file.buffer, { type: "buffer", cellDates: true });
        // Pick data sheet — prefer "SSLC_Students", skip helper sheets starting with _
        const sheetName = workbook.SheetNames.find(n => n === "SSLC_Students") || workbook.SheetNames.find(n => !n.startsWith("_")) || workbook.SheetNames[0];
        const rows = XLSX.utils.sheet_to_json(workbook.Sheets[sheetName], { defval: null });
